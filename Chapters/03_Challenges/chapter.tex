...
Slowly varying tempo-spectral modulations occur both in speech and music signals and are a combination of  amplitude and frequency modulation (See further details given in Section~\ref{sub:time-variant-audio-signals}).
\par
Exploiting slow modulations is natural for humans. 
While~\cite{zwicker52, fastl90} focused on the ability to detect modulations,it was shown by Bregman and McAdams in~\cite{mcadams89, bregman90} that humans make use of the concept called \emph{Common Amplitude Modulation} (CAM) to segregate sources in mixture. 
The CAM effect exploits the fact that harmonics that share the same amplitude modulation across the bins, are perceived as \emph{integrated} as opposed to~\emph{segregated}.
In was shown in~\cite{bacon89} that this ability of amplitude modulations can be modeled.
Furthermore, as found by Dau in~\cite{dau99}, humans are especially sensitive at slow modulations:
...
Dau, in fact, created a model that mimics the humans ability to detect modulation patterns and pointed out applications to improve hearing of hearing-impaired listener or speech intelligibility.
\par
If modulations are so important for humans, then possible research questions for the scope of this thesis are:

\begin{itemize}
  \item Can modulations be automatically detected or extracted from highly overlapped signals?
  \item If modulations are known, can it be exploited to improve tasks like source separation or source count estimation?
...
In order to address these questions, previous research has been doing x, y, z in for processing and analysis using models.
...
There exist a number of method that utilize spectro-temporal modulation to process or separate mixtures.
...
In speech, techniques to utilize modulation patterns improved applications such as speech discrimantion~\cite{mesgarani04} or extract spatial acoustic signatures from mixtures~\cite{sukittanon06}.
