...
In order to make an attempt to formalize the \emph{level of overlap}, a time-frequency representation as introduced in Section~\ref{sub:time-frequency-representation} and how they are typically applied to address the source separation problem.

\hypertarget{separability-of-mixtures}{%
\section{Separability of Mixtures}\label{separability-of-mixtures}}

Time frequency representations such as the short-time Fourier transform (STFT) have obvious benefits such as the improved interpretability due to its ``image-like'' two-dimensional properties.
...
% TODO: check
It is a reasonable approach where it is usually assumed that an STFT provides sufficient level of separability (e.g.~by given a perfect mask).
...
Further, it was shown in~\cite{bacon89} that the ability of detecting amplitude modulations can be incorporated into auditory models.
...
% TODO: maybe move to intro, objectives?
Dau then proposed a model that mimics the humans ability to detect modulation patterns and pointed out applications to improve hearing of hearing-impaired listener or speech intelligibility.
\par
Previously, research has address a variety of tasks of processing and analysis in the context of modulations.
...
% [ ] Transition sentence that acts as a “teaser” for the next chapter

Modulations play an important role in audio signals. 
...
This comes with a number of challenges: Firstly, parameterization of modulation characteristics of a single source is difficult, when only the mixture can be observed. 
For example, it is known~\cite{salamon13} that the extraction of the fundamental frequency in a mixture is challenging. 
The reason is that crossing partials are a challenging problem for sinusoidal modeling~\cite{viste03}. 
And even if tracking of such them would work perfectly, evaluation of robustness and accuracy is often hardly possible when the reference data is only annotated with human precision.
Secondly, representations like modulation spectrograms only cover amplitude modulations, whereas generic modulation patterns (AM/FM, timbre modulation) cannot be covered.
% TODO: Highly overlapped signals require suitable representations improves separability compared to STFT.
\par
...
In the next two chapters, I will address this questions by utilizing the modulations of sources for separation tasks; either via prior knowledge (known) or by operating blindly (unknown).
