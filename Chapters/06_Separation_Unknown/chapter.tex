...
Often this can be achieved by some similarity metric that allows to calculate pairwise distances between the \(K\) components and the desired sources and applying \textit{k}-means clustering algorithm~\cite{spiertz09}.
...
In \cite[Chapter 16]{VVG18b} of that book the authors describe one of the main problems of NMF which is that ``standard NMF is shown to be efficient when the notes of the analyzed music signal are nearly stationary''.
...
Therefore, a variety of models exist to factorize a tensor into three factors and apply a similar rank reduction as in the NMF case.
Tensor factorizations allow to incorporate more complex models and are therefore useful for applications with more than two dimensions.
...
However, we found that it is easier follow when the individual tensor elements are used, which is also the recommended notation proposed in~\cite{kiers00}, for  $f = 1,\ldots,F; b=1,\ldots,B;t=1,\ldots,T$:
...
\marginpar{This section is based on the work that has been published in 2016~\cite{stoeter16}. The original idea was developed with Antoine Liutkus (INRIA, France) and the publication was written with the support of Roland Badeau (ParisTech, France) and Paul Magron (ParisTech, France)}
...
\section{Common Fate Transform for Lead Accompaniment Separation}%
\label{sec:cft_for_lead_accompaniment_separation}

\marginpar{This section is original work being done as part of the \emph{STO1} and \emph{STO2} submission for the SiSEC 2016~\cite{sisec16}. Part of the literature overview is taken from~\cite{rafii}.}
...
In this section, we want to show how this method can be extended for the application of lead and accompaniment separation~\cite{rafii}.
...
However, most systems today still use classical time-frequency representations, to build time-frequency (TF) masks.
% end zafar
\par
...
Different architectures for neural networks may be combined/cascaded together, and many architectures were proposed in the past, such as feedforward fully-connected neural networks (FNN), convolutional neural networks (CNN), or recurrent neural networks (RNN) and variants thereof such as the long short-term memory (LSTM) and the gated-recurrent units (GRU).
...
The aim of this work was not to exactly reproduce the results of~\cite{uhlich15} but instead, to evaluate one main research questions: does a DNN-based model benefit from the common fate representation being able to better capture the modulation texture?
...
\subsection{Training}
W then sample from the DSD100 tracks and form single samples as input for the FNN.
Therefore, we first compute the input representation of an audio track from the DSD100 set and then create samples by randomly sampling without replacement from these tracks.
...
\marginpar{Parts of this subsection have been published in the SiSEC anlysis section of~\cite{rafii} which was co-organized by the author of this thesis.}
...
Finally, we presented interactive evaluation tools, developed for SiSEC, that allow to interactively assess the performance of separation systems both, objectively and subjectively.