...
\marginpar{This chapter is based on the work that has been published in 2016. The work has been done in close collaboration with Antoine Liutkus from INRIA, France and Roland Badeau from ParisTech France.}
...
\section{Common Fate Model for Lead Accompaniment Separation}
...
Most systems today use classical TF representations and building TF masks.
Although end-to-end systems will inevitably be used near future, the common structure of deep learning methods for lead and accompaniment separation usually corresponds for now to the one depicted in Figure~\ref{fig:methods_dnn}. From a general perspective, we may say that most current methods mainly differ in the structure picked for the network, as well as in the way it is learned.

\begin{figure}
  \centering
  \includegraphics[width=\columnwidth]{Chapters/06_Separation_Unknown/figures/methods_dnn.pdf}
...
Providing a thorough introduction to deep neural networks is out of the scope of this thesis. For our purpose, it suffices to mention that they consist of a cascade of several possibly non-linear transformations of the input, which are learned during a training stage. They were shown to effectively learn representations and mappings, provided enough data is available for estimating their parameters \cite{deng14,lecun15,goodfellow16}.
...
I decided to use the network architecture of~\cite{uhlich15} to assess the separation quality when used togeter with the common fate representation.
Therefore, I developed a simplified implementation of~\cite{uhlich15} using the Keras deep learning framework.
...
\begin{itemize}
  \item DNNs: Super-Frames are not relevant anymore
  \item DNNs: Modern Architechtures are more powerful: SiSEC 2018
\end{itemize}
