...
% from barker
Factorisation is essentially an optimisation problem. Factors are chosen so that the error between the target and its estimation is minimised. A model which minimises some cost function does not necessarily ensure good source separation quality, so the cost function for a particular problem should be sensibly designed.
\begin{equation}
  \min _ { \mathbf { W } , \mathrm { H } } D ( \mathbf { V } | \mathbf { W } \mathbf { H } ) \text { subject to } \mathbf { W } \geq 0 , \mathbf { H } \geq 0
\end{equation}
...
The special cases of \(\beta = 0\) and \(\beta = 1\) correspond to the Itakura-Saito (IS) and Kullback-Leibler (KL) divergence, but also Euclidean distances (\(\beta = 2\)) is used in practice.
...
\par
To underpin this issue, I depict this problem in Figure~\ref{fig:am_tensor}, where I show the factorization of a simple amplitude modulated input signal. 
...
One way to towards better separations is to increase the number of components per source, however this introduces difficulties for the clustering.
...
To utilize the third dimension, a variety of tensor factorization models exist to factorize the tensor into multiple factors and apply rank reductions --- for a detailed overview the reader is referred to~\cite{cichoki09}.
...
For example, if we mix together two harmonic signals having a fundamental frequency of 440Hz where one signal has a stationary amplitude modulation of 4~Hz and another one of 10~Hz, these differences are latent in a non-negative time frequency representation.

% TODO: By extending both, the representation as well as the factorization model, the results in some kind of end-to-end modulation based model.
...
The overall performance is depicted in~\cite{stoeter14}.
