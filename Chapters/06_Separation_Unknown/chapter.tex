...
% TODO: NMF With only one channel of sound, we can take the magnitude spectrogram, which is a non-negative matrix !  NMF takes this matrix and iteratively approximates it with two simpler matrices that multiply together to produce the original matrix !  The approximation is driven by a cost function !  Rows and columns have special meanings that allow recovery of a mask that can be applied to the spectrogram to recover the sources
