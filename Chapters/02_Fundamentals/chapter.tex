...
Therefore, we can observe specific properties, consistent with the limitations of the human hearing system, for example in dynamics/loudness as well as a limited signal bandwidth.
...
Since the hearing range of normal listening humans is 20~\si{\hertz} - 20~\si{\kilo\hertz}~\cite{fastl90, moore89}, due to the Nyquist-Shannon sampling theorem, sample rates at least 40~\si{\kilo\hertz} are often chosen to facilitate the full human hearing range.
...
The signal becomes time-variant when an external function changes the parameters of a signal over time.
These are the basis for many break-through inventions such as radio transmission~\cite{shannon48} and usually referred to as \emph{modulations}.
...
\begin{description}
  ...
  \item[2) Raw Source Image:] the digital raw source signals are grouped and mixed to into a \emph{source image} (also \emph{stem}).
...
Methods based on panning information, separate sources based on these assumptions. 
Such approaches typically compare the left and right channels of a mixture in the time-frequency domain.
\par
...
The separation of music signals in two parts: the foreground lead (mostly the vocals) and the background accompaniment (drums, bass, other) is one of the most relevant scenarios in music separation with a large number of applications such as mixing karaoke or a capella tracks.
...
\par
The \emph{number of sources} \(k \in \sN\) appears to be a clearly defined property of a mixture. 
