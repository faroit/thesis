...
In many ways, mixtures are not different to any other audio signal, meaning that one can envision a countless number of audio related processing and analysis tasks.
However, two tasks stand out they are would be answer two very basic questions:

\begin{itemize}
    \item Can the process of mixing sources be undone? 
    \item How many sources were involved in mix?
\end{itemize}

To answer these two questions, I want to introduce the two basic scientific research fields of \emph{Sound Source Separation} and \emph{Source Count Estimation}.
...
We apply source separation every day in our life.
...
Due to this large number of contributions, it is not feasible to give an extensive overview on all existing methods in the context and I refer the reader to overview literature with respect to other scenarios when appropriate.
...
In the following present categories of separation scenarios.
