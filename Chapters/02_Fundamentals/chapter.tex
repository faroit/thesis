...
Summarizing, many of modulations audio signals are rather at low frequency, as summarized in Table~\ref{tab:modulations}.
Interestingly one can observe that many of these modulations have a rate of about 5~\si{\hertz}. 
While~\cite{zwicker52} found that our threshold for detecting amplitude modulations is very small at low modulation frequencies, low rates of about 5~\si{\hertz} appear and feel to be somewhat natural for humans.
This observation can be confirmed when looking at physical modulations that occur when humans have desease like vocal tremor~\cite{ramig87} or Parkinson~\cite{botzel14}: in both cases muscle contractions of the same frequency are actuated. 
...
Instead, we are faced with multiple sources that render an \emph{acoustical sound scene}.
In this setting, a scene could described as a \emph{set of sources} of arbitrary size.
\par
...
This \emph{mixture} represents a mapping from a set of sources
...
\par
Usually, the mixing process is assumed to be time-invariant but for a variety of signals such as a live recording with moving sources it can also be time-variant.
...
\begin{description}
  ...
  The source image is mixed to a specific number of output channels (e.g. stereo) even though the recording is done by a mono microphone (e.g. vocals) or multiple microphones (e.g. drums).
  In this stage a panning is added to spatially position either the sources or the image.
