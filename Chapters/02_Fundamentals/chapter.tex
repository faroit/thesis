...
first, the continuous time signal \(x(t)\) is converted to a discrete time series, so that one sample\footnote{Please note, that the use of the word \emph{sample} will have different meanings in the context of machine learning, where a sample is an instance of a full signal instead of a single time step.} \(x_n\) is \emph{sampled} with equidistant steps \(\Tau\); 
...
An important parameter in the process of digitization is the sample rate \(F_s = 1/\Tau\) where \(\Tau\) is the sampling period.
...
\par
STFT matrices \(\mX \in \sC^{T \times F}\) are complex and include phase information.
