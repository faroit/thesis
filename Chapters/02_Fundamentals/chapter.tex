...
The objective equivalent is referred to as the fundamental frequency (\(F_0\)) of a signal\footnote{Pitch and $F_{0}$ are often used synonymously in audio research. Even though this is incorrect I will sometimes refer other work where pitch is instead of $F_{0}$.}.
...
For a a detailed overview into the research field of pitch and \(F_{0}\), the reader is refered to~\cite{decheveigne05, klapuri06book, salamon13}.
\par
An estimate of the fundamental frequency $F_{0}$ of a signal is required in various applications of audio and speech signal processing.
...
However, the most common scenario in many works is to extract the fundamental frequency of a monophonic and harmonic audio signal containing speech or music~\cite{talkin95, boersma02, decheveigne02, resch07, camacho07, tidhar10, christensen07, stoeter15icassp}.

\subsection{Modulations}

\begin{itemize}
    \item Purpose of modulation
    \item type of mudulation
    \item extend of modulation
    \item speed
    \item shape
\end{itemize}
from telecommunication where the modulator is a signal that modifies a carrier function for the purpose of carrying information. 
