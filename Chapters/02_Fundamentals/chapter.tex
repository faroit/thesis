...
The objective equivalent is referred to as the \emph{fundamental frequency} (\(F0\))\footnote{Pitch and $F_0$ are often used synonymously in audio research. 
...
For a detailed overview into the research field of pitch and \(F0\), the reader is referred to~\cite{klapuri06book}.
...
This simple idea was the basis for many break-through inventions such as radio transmission~\cite{shannon48}.
...
If we imagine a sinusoidal carrier signal $x(t) = \cos \omega_c t$, \emph{amplitude modulation} (AM) is applying a time varying amplitude $a(t)$ so that:
...
where A is the amplitude, $\omega_0$ is the carrier frequency, $\omega_m$ is the instantaneous modulation frequency, and $m$ is the modulation index $m = \frac{\Delta \omega_0} { \omega_{m} }$.

The Fourier spectrum of $s_{FM}(t)$, which depends, in theory, on Bessel functions that do not admit a closed-form expression, is intractable in practice~\cite{abramowitz64}.
...
Vocal vibrato rates are similar to that of instrumental vibrato with its peak around 5~\si{\hertz}.
