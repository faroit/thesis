...
Furthermore source separation is difficult to evaluate and researchers compare to a known reference even though this doesn't reflect how humans separate mixtures.
In fact, it has long been a research topic to answer the question \emph{if} humans are able to separate, by means of fully extracting the desired source or if we can focus our attention to one source --- segregate them~\cite{bregman}.
And, despite recent progress in auditory science~\cite{carlyon04, koelsch05, rabinowitz13}, research still is investigating how separation takes place in auditory cortex or other parts of the human brain.
One thing, however, we are able to assess directly, is if humans can reliably detect the number of sources in a mixture of several sources.
\par
In order to address this question, one must ask how humans generally infer counts and if our strategy depends on the number to be counted.
When we look at vision, these are questions that have already been discussed over hundred years ago in scientific research. 
To my knowledge, the earliest study in psychology of vision was published in 1871 by Jevons~\cite{jevons1871}. 
He presented an experiment to count the number objects (beans) and came up with the hypothesis that humans can instantly estimate the number of objects without actually counting and therefore identifying them (``counting by detection'').
...
This ``one-two-three-many'' hypothesis was a fundamental observation and since then, the fact that we can directly infer the numerosity of small numbers of objects, up to about four, is known as \emph{subitizing}~\cite{kaufman49, burr10}.
I will also refer to this strategy as ``direct count estimation''.
Regarding our hearing, there are indications that the auditory system is capable of subitizing audio sources~\cite{hoopen79}.
...
% TODO: Put tables into Appendix

\section{Experiments on the Instrument Count Estimation}%
\label{sec:ismir}
...
\center
\small
\begin{tabular}{cr@{.}lr@{.}lp{5.8cm}c}
\toprule[1.5pt]
...
% TODO: musicians are known to be better in many tasks: ~\cite{kishon01}
...
Signals are linearly mixed according and peak normalised to avoid clipping.
...
A simplified and shorter version of the experiment is made available through a web application\footnote{\url{https://denumerate.app}}.
...
Another observation refers to the fact that the informed participants did perform better overall even though lower speaker counts such as $k=3,4$ should not be affected.
...
Source Count estimation can be easily set up. 
...
~\cite{huron89} Reaction times: ``it takes five times longer to count to the number five than it does to count to the number one.''.
