...
The extent of such meta information very often depends on the availability the data.
...
\marginpar{This section is based on the work that has been submitted in 2014 and together with my collegue Stefan Bayer ~\cite{stoeter14}. The publication is based on my original idea.}
...
To separate the components from the warped mixture we used NMF on a STFT computed with a very long DFT (about 0.5 s). 
NMF can work unsupervised by detecting the more tonal $\textbf{W}$ component by using a spectral flatness measure. The separated signals (f) and (g) then need to be warped back into the original time domain resulting in (h) and (i).
...
That way, the full signal is unwarped and resampled to the same pitch, which then only requires a single comb filter to extract the signal.
...
\end{tabular}
\caption{Examples of a F0 variation informed separation of the audio track \emph{Tamy - Que Pena Tanto Fa}.}%
...
In this chapter, I highlighted the time-varying aspects of musical sources such as vibrato for the use in source separation applications.
To address this scenario, we developed a method that utilizes time warping to more easily extract a source from the mixture.
...
In the warped time domain the frequency modulation of the desired source is canceled. 
...
\par
In order to evaluate if the our method can be applied to a more realistic scenario, we proposed an extension of the method for the scenario of vocal/accompaniment separation.
We used a state-of-the-art melody estimation technique to extract the F0 variation of the vocal source to apply the warping on the mixture.
We performed separation in the time domain using a comb filter to further reduce artifact.
When the method was applied on the DSD100 dataset, several limitations became apparent which is why the overall performance of the system was not satisfying in this scenario.
The main limitation of the method is that it relies on an precise and robust estimate of both, the fundamental frequency estimate as well as vocal activity estimate. 
\par
To address the latter, I extended the method to include a deep neural network based vocal activity detector.
...
The proposed method applies time warping iteratively based on an initial F0 estimate. 
Therefore the implementation can be applied to any F0 estimator as a post-processing step.
\par
To conclude, time warping based separation can work surprisingly well on some signals but requires further hand crafted tuning to yield good results.
However, the presented concepts can still be applied to todays data-driven models as time warping is already applied in current state-of-the art loss functions~\cite{graves06} for speech recognition models~\cite{amodei16}.
...
% TODO: add limitations of warped F0
% However, we have to make clear that the improvements in accuracy are not Maybe in combination with high accuracy dataset such as the one we developed in~\cite{stoeter15acm}
% time warping techniques can also be used 


% my own conclusions
% * refined pitch estimation needs optimization criterion
% * 
