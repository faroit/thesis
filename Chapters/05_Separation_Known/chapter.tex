...
From this starting point, two kinds of approaches can be distinguished between, depending on how they exploit the pitch information.
The first option to obtain the separated lead signal is to re-synthesize it using a sinusoidal model, which has been studied in several approaches which are not detailed in this thesis (for a detailed overview of these methods, I refer the reader to Section III A of~\cite{rafii}).
...
\begin{figure}[htbp]
  ...
  \includegraphics[width=\columnwidth]{Chapters/05_Separation_Known/figures/comb_filter.pdf}
	\caption{The approaches based on a \textit{harmonic assumption} for vocals. In a first analysis step, the fundamental frequency of the lead signal is extracted. From it, a separation is obtained by filtering the mixture.}
...
The melody can be defined in~\cite{poliner07} as:
...
For a more comprehensive overview into melody extraction methods the reader is referred to in~\cite{salamon14}.
In this work (Chapter 1, Section 1.4.5), Salamon also describes some advantages of a melody extraction method without applying a source separation system first as ``Understanding without Separation''.

In this work I used the \emph{Melodia} algorithm publish by Salamon in 2012~\cite{salamon12}.
...
\textbf{2)}: the spectral peaks are the basis to generate a saliency spectrogram using weighted harmonic summation. This allows emphasize the predominant/salient frequencies in the signal and is the core part of the \emph{Melodia} algorithm.
\textbf{3)}: From the saliency map, again, peaks are extracted and connected to a melody line. This is already a good starting point for the melody estimate but usually contains way to many outliers due to the noisiness of the saliency representation.
\textbf{4)}: The melody line is post-processed using a viterbi decoder.
This allows to filter the contour by removing outliers, pitch jumps and generally improves the smoothness of the contour using a number of heuristics.
It is important to note that this step is tracking the melody line of the previous step over time.
Usually this step is sensitive to the overall length of the processed mixture and often, this step is computed an semantically meaningful segment of the mixture like a full track or a refrain.
\par
I applied the algorithm using the implementation in Essentia~\cite{bogdanov13} using the default settings (sample rate 22050 Hz, hop size = 3 ms, window size=46 ms).

\subsection{Source Extraction using Time Warping}

Once the predominant melody is obtained, the warp contour can be computed in the same way as described in Section~\ref{sub:time_warping}.
Since the

One obvious drawback of our method is that it relies on a robust pitch variation estimator which is usually difficult to obtain from a mixtures where multiple harmonic sources are present at the same im.
...
\textbf{4) Segmental Warping}:
\textbf{5) Pitch Contour}:
...
The algorithm is implemented in Python and has low complexity resulting in approximately 0.33s for 1s of audio on 3,1 GHz

Instead of a global warping function we apply the warping locally using VAD.
