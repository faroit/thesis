...
\marginpar{This subsection is based on parts of the work that has been published in~\cite{rafii} with minor contributions of the author of this thesis.}
...
\marginpar{This section is based on the work published in 2014 together with my colleague Stefan Bayer~\cite{stoeter14}. The first author did mainly contribute to the work.}
...
With the ability to remove the frequency modulation from a signal, we included this system in a source separation system to address the non-stationarity issues of NMF based approaches. 
...
It is important to clarify that this approach would not be able to separate two modulating instruments playing in unison without having prior knowledge about the individual modulation functions. Although a $F0$ variation estimate might be difficult to achieve in a mixture, our approach shows that such a system works if that estimate is accurate.
...
% The procedure 
% from zafar
% Hereby we make use of one particularity of harmonic lead sources like singing voice.
...
% Such algorithms typically assume that the lead corresponds to the harmonic signal with the strongest amplitude.
...
% Predominant melody estimation is built upon \(F0\) estimates by applying ``a second step then is respect is then to track this fundamental frequency over time, in other words, to find the best sequence of estimates, in order to identify the melody line.''~\cite{rafii}.
...
% (e) Warped Mixture & (f) Filtered and Warped Mixture \\[6pt]
...
\par
% Locking back, at that time, we did not have enough knowledge to understand how and why deep learning method are so successful~\footnote{And even today, very few people would say that the deep learning community made significant progress regarding the understanding of DNNs.}.
Therefore, we decided to integrate a state-of-the-art singing voice detection system into the separation pipeline and evaluate just the end-to-end performance.
...
% We want to briefly describe the system published in in~\cite{Leglaive15}, for more details, the reader is referred to the original author's publications.
...
% This makes it ideal to model time series, however, in practice, the temporal context learned is limited to only a few time instances, because of the vanishing gradient problem~\cite{Hochreiter98}.
% To alleviate this problem, forgetting factors were proposed.
% One of the most popular gated recurrent cells is the Long Short-Term Memory (LSTM)~\cite{Hochreiter97} cell.
% Its effectiveness has been proven in various applications and LSTMs are the state-of-the-art approach to speech recognition~\cite{Graves13}.
...
%   \label{fig:lstm_2}
% \end{figure}
The input of~\cite{Leglaive15} is a 80-dimensional feature vector consisting of harmonically and percussively enhanced STFT frames of the mixture as described in~\cite{ono08}.
...
The trained network achieves an accuracy of 85\% on the test set.\par
...
\section{Improving $F0$ Estimation Using Time Warping} % (fold)
\label{sec:f0method}

\input{Chapters/05_Separation_Known/stats}

\marginpar{This section is based on the work that has been published in 2015 together with my colleague Nils Werner~\cite{stoeter15icassp}, based on the original idea of the first author.}
...
In our application, the warp map $w(t)$ is constructed in such a way that the instantaneous changes in frequency of the signal in the linear time domain are minimized in the warped time domain. For this, we derive the map from an estimate of the fundamental frequency $F0$ using Equation~\ref{eq:time_warp} from Section~\ref{sub:frequency_modulation}.
