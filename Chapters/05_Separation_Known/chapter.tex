...
The first option to obtain the separated source is to re-synthesize it using a sinusoidal model, which has been studied in several approaches which are not detailed in this thesis (for a detailed overview of these methods, I refer the reader to Section III A of~\cite{rafii}).
...
The algorithm has been applied on the Mixing Secret Dataset 100 (MSD100) consists of a total of 100 songs of different styles.
The separation results were evaluated using BSS_eval~\cite{} and submitted to the SiSEC 2015 evaluation challenge~\cite{ono15}.
...
Hence, they were not detected by the \emph{Melodia} voice activity detection.
...
As mentioned before, I identified that the voice activity detection method takes an important part in the proposed separation system.
Therefore I decided to improve the robustness of the system by improving the voice activity detection algorithm.

Jamendo Singing Voice Detection Dataset~\cite{ramona08}.

\cite{Ramona} Accuracy 82.2, F-Measure 84.3
\cite{Leglaive15} Acc 91.5, F-Measure 91.
ich Acc 83.9, 83.6

30, 20, 40 layer, but unidirectional.
sequency of frames being singing voice or not.
...
LSTM (lstm)                   (None, None, 30)              13320
LSTM (lstm)                   (None, None, 20)              4080
LSTM (lstm)                   (None, None, 40)              9760
TimeDistributedDense (timedist(None, None, 1)               41

Linear activation instead of Dense

I adopted~\cite{leglaive15} and
...
\marginpar{This section is based on the work that has been submitted in 2015 together with my colleague Nils Werner~\cite{stoeter15icassp}.}
