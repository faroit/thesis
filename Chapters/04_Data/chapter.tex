...
\begin{itemize}
  ...
  \item When notes were played with vibrato, having access to the individual modulation patterns can help to study the influence of modulations in systematic fashion.
...
Synthetic data sets are a great tool for systematic evaluations. However, in real-world scenarios, additional components such as recording conditions play an essential role which may degrade the algorithm’s performance. In the following sections, we present two data sets which aim to fill this gap and which will be used in subsequent experiments.
...
The retrieved F0-trajectory based on the EGG signal is easier to process and the generated annotations are considered as a very close ground truth~\cite{pirker11, babacan13}.
