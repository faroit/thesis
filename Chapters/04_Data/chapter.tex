...
\label{cha:datasets}
In the previous chapter applications and problems in the context of modulations of highly overlapped signals were presented.
Before I present methods that separate or analyze such signals, I find it helpful to study and understand the importance of data with these properties.
...
Even though modulation is an intrinsic property of many audio recordings such as speech and music, very few dataset are available to actually study influence of the modulation into various tasks such as source separation.
Furthermore, datasets are also required for evaluation purposes.
\par
In the following I will give three examples for datasets that were created in the course of this thesis.

In music however, recordings are usually not available as separated tracks due to legal issues.
And very few publicly available datasets exist that include separated tracks.
...
By contrast, the conversational aspect of human-to-human communication is lost, which means does not assemble a realistic cocktail party nor a conversation.
There are use-cases for single note datasets such as for the evaluation of F0 estimation algorithms or to identify the instrument.
...
And, compared to speech, musical content usually does share common orchestration and can usually not superimposed in a random way, which makes simply summing isolated random notes from instrumental databases to produce mixtures.
However, when all instruments playing \emph{in unison} to create mixtures with very high overlap, e.g. single note datasets are useful and also helpful for two reasons:

\begin{itemize}
  \item A random summation of multiple instruments playing the same note is musically meaningful mixtures.
...
We propose a scenario where instruments play in unison. 
...
In a small experiment, we computed the average \(WDO\) for 1000 random combinations of mixtures for each scenario.
