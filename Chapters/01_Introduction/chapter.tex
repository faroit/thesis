...
Since almost 70 years~\cite{cherry53}, research is fascinated by this idea to computationally imitate this ability of humans to separate the sources in a mixture.
...
As we illustrated, humans are not very good at estimating the number of sources based on audio which sharply contrasts with our ability to focusing on a single source in a crowded audio scene.
\par
...
For this, we first study and develop new representations that could improve the analysis and processing of such signals, to address if modulations be automatically detected or extracted from highly overlapped signals.
...
Along the way, I also proposed a post-processing to improve \(F0\) estimates based on the same principles.
...
This thesis is based on the previously listed publications. As such, they are cited repeatedly throughout this thesis. If a section is mainly based on one of this publication, a remark is added at the side of the page, instead of citing the same publication exhaustively. If more authors than me and my supervisor contributed, they are mentioned there, and the main contributions are specified.
