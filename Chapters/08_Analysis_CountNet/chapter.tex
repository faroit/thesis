...
Many audio-related applications like speech recognition~\cite{HintonSpeech} or speaker diarization share similar architectural structures, often found by incorporating domain knowledge and through extensive hyper-parameter searches.
...
The mixtures were generated as described in Section~\ref{ssec:stimuli}.
...
Based the voice activity detection algorithm (VAD), we computed the ground truth output \(\cardinality \) via (\ref{eq:definition_k}).
...
For a more detailed description of the dataset, the reader is referred to~\cite{stoeter17}.
...
That way, for each epoch, the network is seeing slightly different samples, reducing the number of redundant samples and thus helping to speed up the stochastic gradient based training process\footnote{Note that for the validation and testing, excerpts are fixed.}.
...
Concerning the input representation, we can report that using \acs{STFT} representation outperforms the log-scaled \acs{STFT} as well as the MEL representation.
