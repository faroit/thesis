...
It shows that specialized representations, designed to capture general modulation patterns, is still useful in data-driven methods.
...
With the success of deep learning, a paradigm shift took place that enabled many improvements with respect to state-of-the-art in the audio domain. However, adapting deep learning techniques to work in the audio domain is far from trivial. First, it requires a lot of work in creating suitable data sets. Second, it still takes expertise in so-called “classical” signal processing for such models to work correctly.
% I think you don't want to end your manuscript by telling old guys you had to drop classical signal processing. Moreover, I think this is not exactly true: the challenge rather lies in the way to go from these fixed tricks that are supposedly optimal in average to learnable, data-adapted methods that actually work better for your actual datasets.
...
Using such modules for estimating counts hence seems like a natural follow-up to the work presented here.
% I believed you laked the ending sentence for this
